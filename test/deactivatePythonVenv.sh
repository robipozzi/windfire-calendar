...
main()
{
    ...
    if [ -d "$PYTORCH_VIRTUAL_ENV_TEST" ]; then
        echo "Python virtual environment ${blu}$PYTORCH_VIRTUAL_ENV_TEST${end} exists, deactivating ..."
        deactivate
    else
        echo "Python virtual environment ${blu}$PYTORCH_VIRTUAL_ENV_TEST${end} does not exist"
    fi
}
